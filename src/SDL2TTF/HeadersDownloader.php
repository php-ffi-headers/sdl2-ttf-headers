...
final class HeadersDownloader
{
    ...
    {
        return \vsprintf('https://github.com/libsdl-org/SDL_ttf/archive/refs/tags/release-%s.zip', [
            $version->toString(),
        ]);
    }
    ...
    {
        return \sys_get_temp_dir() . '/sdl2-ttf-headers-' . $version->toString() . '.zip';
    }
    ...
    public static function download(VersionInterface $version, string $directory): void
    {
        ...
        foreach (self::readArchive($version) as $file) {
            if ($file->isFile() && \str_ends_with($file->getPathname(), 'SDL_ttf.h')) {
